...
classes.panels.Modifiers = function () {
	...
	function createClick () {
		Dialog.create({
			size: "500x*",
			title: Lang.lookup("Create Modifier"),
			content: classes.dialogs.ModifierEditor,
			init: {
			},
			owner: this,
			options: [{
	...
	function editClick (obj) {
		Dialog.create({
			size: "500x*",
			title: Lang.lookup("Edit Modifier"),
			content: classes.dialogs.ModifierEditor,
			init: {
				label: obj.label,
				price: obj.price,
	...
	function draw () {
		...
		var xhtml = '';
			...
			xhtml += 			'<div class="Button Create"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span> ' + Lang.lookup("Create New Modifier") + '</div>';
		...
		_table = $('#' + _uniquid + " table").DataTable( {
			...
			language: {
				emptyTable: Lang.lookup("No Papers Created")
			},
			rowReorder: {
				update: false,
				dataSrc: 'index'
		...
		_setlist.name("modifier");
		_setlist.puid(_state[1]);
		_setlist.columms(["label", "price", "shipping"]);
...
classes.panels.Modifiers.prototype = new EventDispatcher();
