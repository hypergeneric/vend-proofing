...
classes.panels.Formats = function () {
	...
	function createClick () {
		Dialog.create({
			size: "500x*",
			title: Lang.lookup("Create Format"),
			content: classes.dialogs.FormatEditor,
			init: {
			},
			owner: this,
			options: [{
	...
	function draw () {
		...
		_setlist.name("format");
		_setlist.puid(_state[1]);
		_setlist.columms(["label", "width", "height", "price", "shipping"]);
...
classes.panels.Formats.prototype = new EventDispatcher();
